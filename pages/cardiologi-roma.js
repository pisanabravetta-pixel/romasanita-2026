...
export default function CardiologiRoma() {
  ...
  // Recupera schemi e FAQ per Cardiologi a Roma
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', backgroundColor: '#f0f4f8', minHeight: '100vh', color: '#1a202c' }}>
      <Head>
        <title>Cardiologi a Roma | Visita Cardiologica ed ECG | ServiziSalute</title>
        <meta name="description" content="Cerca i migliori cardiologi a Roma. Prenota una visita cardiologica con ECG, Holter o ecocardiogramma nei migliori studi specialistici della Capitale." />
      ...
      <div style={{ backgroundColor: '#0284c7', color: 'white', padding: '12px 0', textAlign: 'center', fontSize: '14px', fontWeight: 'bold' }}>
        ❤️ SPECIALISTI IN CARDIOLOGIA A ROMA
      </div>

      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '20px' }}>
        <a href="/" style={{ display: 'inline-block', marginBottom: '20px', color: '#0284c7', textDecoration: 'none', fontWeight: '600' }}>← Home</a>

        {/* BOX INTRODUTTIVO */}
        <div style={{ backgroundColor: 'white', padding: '35px', borderRadius: '24px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', marginBottom: '30px', borderLeft: '8px solid #0284c7' }}>
          <h1 style={{ color: '#0369a1', fontSize: '32px', margin: '0 0 10px 0', fontWeight: '800' }}>Cardiologi a Roma</h1>
          <p style={{ color: '#4a5568', lineHeight: '1.6', fontSize: '16px', maxWidth: '800px' }}>
            Trova i migliori specialisti in cardiologia a <strong>Roma</strong>. In questa sezione puoi consultare i profili di medici esperti per visite cardiologiche, elettrocardiogramma (ECG) e monitoraggio della pressione arteriosa nei principali quartieri di Roma.
          </p>
        </div>
        ...
        {loading ? (
          ...
          medici.map((v) => (
            <div key={v.id} style={{ backgroundColor: 'white', padding: '30px', borderRadius: '24px', marginBottom: '20px', border: v.is_top ? '3px solid #0284c7' : '1px solid #e2e8f0', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <h2 style={{ color: '#0369a1', margin: '0', fontSize: '24px', fontWeight: '800' }}>{v.nome}</h2>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '25px' }}>
                <a href={`tel:${v.telefono}`} style={{ flex: 1, backgroundColor: '#0284c7', color: 'white', padding: '16px', borderRadius: '16px', textAlign: 'center', fontWeight: 'bold', textDecoration: 'none' }}>Chiama</a>
                {v.whatsapp && (
          ...
          <p style={{ textAlign: 'center' }}>Nessun cardiologo trovato al momento.</p>
        )}

        {/* FAQ SECTION */}
        <section style={{ marginTop: '50px', backgroundColor: 'white', padding: '35px', borderRadius: '24px', marginBottom: '50px' }}>
          <h3 style={{ color: '#0369a1', fontSize: '24px', marginBottom: '20px' }}>Domande Frequenti (FAQ)</h3>
