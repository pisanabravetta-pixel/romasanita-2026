...
<div style={{ marginBottom: '20px', padding: '0 5px', fontSize: '15px', fontWeight: '700', color: '#475569', display: 'flex', alignItems: 'center', gap: '8px' }}> <span style={{ backgroundColor: tema.primario, color: 'white', padding: '3px 10px', borderRadius: '6px', fontSize: '13px' }}> {totaleAnnunci} </span> <span> {meta.nomeSemplice.toLowerCase().includes('specialistica') ? 'Specialisti' : meta.nomeSemplice} {(() => { const categoria = meta.cat.toLowerCase(); // Se è una FARMACIA o DIAGNOSTICA, usa il femminile if (categoria.includes('farmac') || categoria.includes('diagnost')) { return totaleAnnunci === 1 ? ' trovata' : ' trovate'; } // In TUTTI gli altri casi (Dermatologi, Dentisti, Cardiologi, ecc.) usa il maschile return totaleAnnunci === 1 ? ' trovato' : ' trovati'; })()} a Roma {meta.zona} </span> </div> )}
