  ...
  useEffect(() => {
    async function fetchDentisti() {
      const { data, error } = await supabase
        .from('annunci')
        .select('*')
        .ilike('categoria', 'dentisti%') // Risolve il problema maiuscole/minuscole
        .eq('approvato', true)
      ...
      <main style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '30px' }}>
          {['Prati', 'EUR', 'Parioli', 'San Giovanni', 'Centro Storico'].map((zona) => (
            <a key={zona} href={`/dentisti-roma-${zona.toLowerCase().replace(' ', '-')}`} style={{ backgroundColor: '#fff', border: '1px solid #cbd5e1', padding: '8px 15px', borderRadius: '20px', fontSize: '13px', color: '#1e40af', textDecoration: 'none', fontWeight: '500' }}>
              Dentisti {zona}
            </a>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div key={v.id} style={{ 
            ...
            <p style={{ color: '#475569', fontSize: '15px', lineHeight: '1.6', margin: '15px 0' }}>
              {v.descrizione || "Studio dentistico specializzato. Contatta la struttura per informazioni."}
            </p>
