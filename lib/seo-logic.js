...
export const getSchemas = (categoriaKey, zona, indirizzo = "Roma") => {
  ...
  if (!seoData[categoriaKey]) {
    return { medical: null, faq: null, faqRaw: [] };
  }

  const data = seoData[categoriaKey];
  const zonaNome = formatZona(zona);
  const faqList = data.faq || [];

  // 2. PREPARAZIONE DATI PULITI PER IL FRONTEND
...
const baseUrl = "https://www.servizisalute.com";
  const pageUrl = zona && zona !== 'roma' ? `${baseUrl}/${data.slug}-${zona.toLowerCase()}` : `${baseUrl}/${data.slug}`;
 return {
    medical: {
