...
export const getDBQuery = (slug) => {
  ...
  const mapping = {
    ...
    // Rimosso 'specialistica' da qui per eliminare la pagina fantasma
  ...
  return mapping[cleanSlug] || { cat: 'NON_ESISTE', spec: 'NON_ESISTE' };
};
  
export const getSchemas = (categoriaKey, zona, indirizzo = "Roma") => {
  // 1. CONTROLLO DI SICUREZZA: Se la categoria non esiste in seoData, blocca tutto
  ...
  return {
    ...
    faq: {
      ...
      "mainEntity": faqList.map(f => ({
        "@type": "Question",
        "name": (f.q || "").replace(/{{zona}}/g, zonaNome),
        "acceptedAnswer": { 
          "@type": "Answer", 
          "text": (f.a || "").replace(/{{zona}}/g, zonaNome) 
        }
      }))
    }
  };
