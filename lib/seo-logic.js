...
export const getSchemas = (categoriaKey, zona, indirizzo = "Roma") => {
  // 1. Paracadute: se categoriaKey non esiste in seoData, usa un oggetto vuoto {} invece di crashare
  ...
  return {
    medical: {
      ...
      "name": `${data.name || 'Servizio Sanitario'} ${zonaNome}`,
    ...
    faq: {
      "@context": "https://schema.org",
      "@type": "FAQPage",
      // 2. SOLUZIONE ERRORE RIGA 39: 
      // Usiamo (data.faq || []) così se faq è undefined, map gira su un array vuoto e non crasha
      "mainEntity": (data.faq || []).map(f => ({
        "@type": "Question",
        "name": (f.q || "").replace('{{zona}}', zonaNome),
        "acceptedAnswer": {
