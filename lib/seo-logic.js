...
export const getSchemas = (categoriaKey, zona, indirizzo = "Roma") => {
  // Se la categoria non esiste in seoData, evitiamo il crash usando un oggetto vuoto
  ...
  return {
    ...
    faq: {
      "@context": "https://schema.org",
      "@type": "FAQPage",
      // PROTEZIONE TOTALE: Se data.faq non esiste, usa [] così il .map non va in errore
      "mainEntity": (data.faq || []).map(f => ({
        "@type": "Question",
        "name": (f.q || "").replace('{{zona}}', zonaNome),
        "acceptedAnswer": {
