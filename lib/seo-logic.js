...
export const getDBQuery = (slug) => {
  if (!slug) return { cat: '', spec: '', colore: '#2563eb' };
  const cleanSlug = slug.replace('-roma', '');
  
  // RADICI BASATE SUI VALORI REALI DEL DB:
  // categoria nel DB = "visite-specialistiche cardiologo" / "visite-specialistiche cardiologi"
  // La radice deve matchare sia singolare che plurale (es. "cardiol" matcha entrambi)
  const mapping = {
    'visite-specialistiche': { cat: 'visite-specialistiche', spec: 'specialista',  colore: '#0891b2' },
    'dentisti':              { cat: 'dentist',               spec: 'dentist',       colore: '#0f766e' },
    'farmacie':              { cat: 'farmac',                spec: 'farmac',        colore: '#15803d' },
    'cardiologi':            { cat: 'cardiol',               spec: 'cardiol',       colore: '#dc2626' },
    'psicologi':             { cat: 'psicol',                spec: 'psicol',        colore: '#7c3aed' },
    'dermatologi':           { cat: 'dermatol',              spec: 'dermatol',      colore: '#be185d' },
    'ginecologi':            { cat: 'ginecol',               spec: 'ginecol',       colore: '#db2777' },
    'nutrizionisti':         { cat: 'nutriz',                spec: 'nutriz',        colore: '#d97706' },
    'oculisti':              { cat: 'oculist',               spec: 'oculist',       colore: '#0369a1' },
    'ortopedici':            { cat: 'ortoped',               spec: 'ortoped',       colore: '#1d4ed8' },
    'diagnostica':           { cat: 'diagnost',              spec: 'diagnost',      colore: '#9c27b0' },
    'servizi-sanitari':      { cat: 'sanitari',              spec: 'sanitari',      colore: '#065f46' },
    'servizi-domicilio':     { cat: 'domicilio',             spec: 'domicilio',     colore: '#ea580c' }
  };

  // Restituisce NON_ESISTE per slug non mappati → usato da [slug].js per il redirect 404
  return mapping[cleanSlug] || { cat: 'NON_ESISTE', spec: 'NON_ESISTE', colore: '#2563eb' };
}
