...
export const getSchemas = (categoriaKey, zona, indirizzo = "Roma") => {
  // 1. Prendiamo i dati della categoria, se non esiste usiamo 'dentisti' come backup
  ...
  return {
    medical: {
      ...
      "name": `${data.name || 'Servizi'} ${zonaNome}`,
      "medicalSpecialty": data.specialty || "Health",
      "areaServed": { "@type": "City", "name": "Roma" },
      "description": `Trova i migliori ${data.name || 'professionisti'} a ${zonaNome}.`,
      "address": {
        "@type": "PostalAddress",
    ...
    faq: {
      "@context": "https://schema.org",
      "@type": "FAQPage",
      // 2. PROTEZIONE: (data.faq || []) evita l'errore undefined .map
      "mainEntity": (data.faq || []).map(f => ({
        "@type": "Question",
        "name": (f.q || "").replace('{{zona}}', zonaNome),
        "acceptedAnswer": {
